...
class ValuationParams:
    ...
    interest_expense: List[float] = field(default_factory=list)  # Interest expense (dollars)
    amortization: List[float] = field(default_factory=list)      # Amortization (dollars)
    ...
    # ---- Additional cost of capital inputs ----
    unlevered_cost_of_equity: float = 0.0  # For APV method (if not provided, will be approximated)
