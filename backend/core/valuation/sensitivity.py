...
from .dcf import calc_dcf_series
...
def get_sensitivity_summary(sensitivity_df: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    ...
    for param_name in sensitivity_df.columns:
        ...
        param_summary = {
            "min_ev": float(ev_series.min()),
            "max_ev": float(ev_series.max()),
            "ev_range": float(ev_series.max() - ev_series.min()),
            "ev_mean": float(ev_series.mean()),
            "ev_std": float(ev_series.std()),
            "ev_cv": float(ev_series.std() / ev_series.mean()) if ev_series.mean() != 0 else 0.0
        }
...
def find_most_sensitive_parameter(sensitivity_df: pd.DataFrame) -> str:
    ...
    """
    summary = get_sensitivity_summary(sensitivity_df)
    
    if not summary:
        return ""
    
    # Find parameter with highest coefficient of variation
    most_sensitive = max(summary.keys(), key=lambda k: summary[k]["ev_cv"])
