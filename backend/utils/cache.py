...
def cached_with_params(max_size: int = 128, ttl_seconds: Optional[int] = None):
    ...
    Returns:
        Decorator function
    """
    cache = ValuationCache(max_size, ttl_seconds)
...
# Specialized caching functions for specific calculation types
def cache_dcf_result(params_hash: str, result: Any) -> None:
    """
    Cache a DCF calculation result.
    
    Args:
        params_hash: Hash of the valuation parameters
        result: DCF calculation result
    """
    key = f"dcf_{params_hash}"
    _global_cache.set(key, result)
...
def cache_monte_carlo_result(params_hash: str, result: Any) -> None:
    """
    Cache a Monte Carlo calculation result.
    
    Args:
        params_hash: Hash of the valuation parameters
        result: Monte Carlo calculation result
    """
    key = f"mc_{params_hash}"
    _global_cache.set(key, result)

def get_cached_monte_carlo_result(params_hash: str) -> Optional[Any]:
    """
    Get a cached Monte Carlo calculation result.
